...
$data=$sql->fetchAll(PDO::FETCH_ASSOC);
  ...
  $get=$_GET['alumini_id'];
    $db=getDB();
    $stmt = $db->prepare("SELECT * FROM `alumini` WHERE  Student_id=:alumini"); 
...
<section id="main-content">
    <section class="wrapper">
       <?php
          echo '<form action="editaluminiprf.php?std_id='.$data->Student_id.'" method="post" enctype="multipart/form-data">';?>
    <table  class="table table-bordered">
        <tr><h2><?php  echo $data->Student_name.' ';
                                           ?> Profile(Alumini)</h2></tr>
        <tr>
            
            <td>
                 <label>  Name:</label><?php echo '<input type="text" class="form-control" value="'.$data->Student_name.'"  name="names">'; ?>
                                                                    
            </td>
           
            <td>
                     <label>Batch:</label>
                   <?php  echo '<input type="text" class="form-control" value="'.$data->Batch.'" name="btch"/> ';?>
        ...
        <tr>
          <td>
              <label>Address:</label>
              <?php echo '<input type="text" class="form-control" value="'.$data->Location.'" name="address"/>';?> 
            </td>
            <td>
            <label>Placed in:</label>
              <?php echo '<input type="text" class="form-control" value="'.$data->Placement.'" name="plcd"/>';?> 
            </td>
