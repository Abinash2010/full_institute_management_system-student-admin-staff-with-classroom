...
$aid=$_GET['a_id'];
 ...
 $submit=$db->prepare("SELECT * FROM `submition` WHERE `Assignment_id`=:aid");
    $submit->bindParam(':aid',$aid,PDO::PARAM_STR);
    $submit->execute();
    $submited=$submit->fetchAll(PDO::FETCH_ASSOC);
                ...
                <div class="contents1" style="margin-top: 50px;">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <tr>
                                <?php
      foreach($submited as $result)
      {
echo '<td>'.$result['Student_id'].'</td>
         <td>'.'<a href="../upload_assets/answer/'.$result['File'].'" Download>'.'Download File'.'</a>'.'</td>
        <td>'.$result['Answer'].'</td>
        
        <td><a href="viewanswer.php?s_id='.$result['Student_id'].'&gp_id='.$get.'&a_id='.$aid.'" class="btn btn-primary">'.'View'.'</a>'.'</td></tr>';
      }
      ?>
                        </tbody>
                        </thead>
